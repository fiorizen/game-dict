...
export class DataSyncManager {
	...
	private initialDataState: { gameCount: number; entryCount: number } | null =
		null;
	...
	private analyzeConflict(
		...
		if (
			(csvGameCount > 0 || csvEntryCount > 0) &&
			(dbGameCount > 0 || dbEntryCount > 0)
		) {
			return {
				hasConflict: true,
	 ...
	 * ユーザー選択に基づく処理実行
	 */
	async performUserChoice(
		choice: DataSyncChoice,
			...
			for (const file of gameFiles) {
				...
				const dataLines = lines.filter(
					(line) =>
						line.trim() &&
						!line.startsWith("#") &&
						!line.includes("category_name,reading,word"),
				);
	 ...
	 * 終了時のユーザー選択に基づく処理実行
	 */
	async performExitChoice(
		choice: ExitSyncChoice,
	): Promise<{ success: boolean; error?: string }> {
		if (!choice.confirmed) {
			return { success: false, error: "User cancelled operation" };
		}
					...
					options: [
						{
							...
							action: "import_csv",
							description:
								"CSVのデータでデータベースを上書きします（作業中データは失われます）",
						},
						{
							label: "既存データを保持",
							action: "keep_db",
