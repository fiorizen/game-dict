...
export class CSVHandlers {
	...
	async exportToGitCsv(outputDir?: string): Promise<string[]> {
		...
		// 1. Export games.csv
		if (games.length > 0) {
			const gamesData = games.map((game: Game) => ({
				id: game.id,
				name: game.name,
				code: game.code,
	 ...
	 */
	async exportToMicrosoftIme(gameId: number): Promise<string> {
		...
		if (entries.length === 0) {
			throw new Error(
				`No entries found for game '${game.name}'. IME export requires at least one entry.`,
			);
		}
	 ...
	 */
	async exportAllGamesToMicrosoftIme(): Promise<{
		filePath: string;
		...
		// Collect all entries from all games with metadata
		...
		const allEntries: EntryWithMeta[] = [];
		let totalEntries = 0;
		...
		const seenCombinations = new Map<string, EntryWithMeta>();
		const duplicateEntries: EntryWithMeta[] = [];
		...
		const uniqueLines = Array.from(seenCombinations.values()).map(
			(entryWithMeta) => entryWithMeta.line,
		);
		const content = `${uniqueLines.join("\n")}\n`;
		const filePath = path.join(exportDir, "all-games.txt");

		await fs.promises.writeFile(filePath, content, "utf-8");
		log.debug("Exported all games Microsoft IME file", filePath);
		...
		if (duplicateEntries.length > 0) {
			const duplicatesContent = [
				...
				...duplicateEntries.map((entryWithMeta) => {
					const originalEntry = seenCombinations.get(
						`${entryWithMeta.entry.reading}:${entryWithMeta.entry.word}`,
					);
			...
			log.debug("Exported duplicates file", duplicatesFilePath);
		}
	...
	async importFromGitCsvDirectory(inputDir: string): Promise<void> {
		...
		const gameFiles = files.filter(
			(f) => f.startsWith("game-") && f.endsWith(".csv"),
		);
	...
	async importFromCsv(filePath: string): Promise<void> {
		...
		if (!game) {
			...
			if (existingGames.length > 0) {
				...
				const existingCodes = existingGames.map((g: Game) => g.code);
				let counter = 1;
		...
		for (const record of records) {
			...
			if (!category) {
				const existingCategories = this.db.categories
					.getAll()
					.filter((c: Category) => c.name === record.category_name);
			...
			if (!category) {
				throw new Error(
					`Failed to create or find category: ${record.category_name}`,
				);
			}
	...
	async importFromImeTxt(
		...
		for (const entry of validEntries) {
			...
			if (!category) {
				const existingCategories = this.db.categories
					.getAll()
					.filter((c: Category) => c.name === entry.categoryName);
			...
			if (!category) {
				throw new Error(
					`Failed to create or find category: ${entry.categoryName}`,
				);
			}
