...
- [x] 仕様変更：終了時の自動CSV書き出しはユーザーに確認した上で行う
  - [x] 最後のDB更新以降に更新が行われている場合のみ、ユーザーにCSV書き出しをするか確認する。それ以外は書き出しを行う必要がないのでそのまま終了する
- [x] バグ修正：IME登録用辞書ファイル出力の品詞の値が間違っている

  - カテゴリ「名詞」を「一般」として出力しているが、「名詞」として出力するのが正
  - カテゴリ「人名」→「人名」
...
- [x] **E2Eテスト真のheadless化完了** (完了)
  - [x] Electronアプリの真のheadless実装（ウィンドウ非表示制御）
  - [x] 環境変数による条件分岐実装（NODE_ENV=test && HEADLESS=true）
  - [x] 37/37テスト成功・大幅な速度向上確認（52.3秒）
  - [x] npm scriptsで実行可能（test:e2e:headless）・既存手法維持
