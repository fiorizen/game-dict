...
class DatabaseConnection {
	...
	getDatabasePath() {
		...
		if (isTestEnv) {
			// テスト環境では専用のパスを使用
			const testDbPath = node_path_1.default.join(
				process.cwd(),
				"test-data",
				"game-dict-test.db",
			);
		...
		if (typeof require !== "undefined") {
			try {
				...
				const prodDbPath = node_path_1.default.join(
					userDataPath,
					"game-dict.db",
				);
				console.log("Using production database:", prodDbPath);
				...
				const fallbackDbPath = node_path_1.default.join(
					process.cwd(),
					"test-data",
					"game-dict.db",
				);
		...
		// Default fallback
		const defaultDbPath = node_path_1.default.join(
			process.cwd(),
			"test-data",
			"game-dict.db",
		);
