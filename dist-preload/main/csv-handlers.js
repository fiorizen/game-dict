...
class CSVHandlers {
     ...
     */
    getCategoryImeName(category, format) {
        if (!category)
            return "名詞";
        ...
        if (categoryName === "組織名")
            return "名詞";
        if (categoryName === "ブランド名")
            return "名詞";
        if (categoryName === "作品名")
            return "名詞";
        if (categoryName === "技術用語")
            return "名詞";
        if (categoryName === "専門用語")
            return "名詞";
        // Default fallback
        return "名詞";
    }
    ...
    async importGamesFromCsv(filePath) {
        ...
        for (const record of records) {
            insertStmt.run(parseInt(record.id), record.name, record.code, record.created_at || new Date().toISOString(), record.updated_at || new Date().toISOString());
        }
    }
    /**
    ...
    async importCategoriesFromCsv(filePath) {
		...
		`);
        for (const record of records) {
            insertStmt.run(parseInt(record.id), record.name, record.google_ime_name, record.ms_ime_name, record.atok_name);
        }
    }
    /**
    ...
    async importFromImeTxt(gameId, filePath) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (parts.length !== 3 && parts.length !== 4) {
                errors.push(`Line ${i + 1}: Expected 3 or 4 tab-separated fields, got ${parts.length}`);
                continue;
            }
