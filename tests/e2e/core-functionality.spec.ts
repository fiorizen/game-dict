...
test.describe("Core Functionality Tests", () => {
	...
	test("ゲームAPI基本操作が動作する", async () => {
		...
		const createResult = await page.evaluate(async () => {
			try {
				await window.electronAPI.games.create({
					name: "APIテストゲーム",
					code: "apitestgame",
				});
				return { success: true };
	...
	test("カテゴリAPI基本操作が動作する", async () => {
		const categoriesResult = await page.evaluate(async () => {
			try {
				...
				return {
					success: true,
					hasData: Array.isArray(categories) && categories.length > 0,
				};
			} catch (error) {
				return { success: false, error: error.message };
			}
	...
	test("エントリAPI基本操作が動作する", async () => {
		const entriesResult = await page.evaluate(async () => {
			try {
				const entries = await window.electronAPI.entries.getAll();
	...
	test("データベース操作の一貫性", async () => {
		...
		const operations = await page.evaluate(async () => {
			try {
				await window.electronAPI.games.getAll();
				await window.electronAPI.categories.getAll();
				await window.electronAPI.entries.getAll();
