...
**UI機能E2Eテスト仕様**
- **ゲーム追加機能**: モーダル開閉→入力→保存→リスト更新→選択可能
- **単語追加機能**: ゲーム選択→モーダル開閉→入力（読み・単語・カテゴリ・説明）→保存→リスト更新
- **検索機能**: 検索テキスト入力→部分一致フィルタリング→結果表示
- **ゲーム切り替え**: セレクトボックス選択→単語リスト更新→追加ボタン有効化
